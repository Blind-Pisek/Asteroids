...
export class Death : public Menu {
	...
	void createTryAgainButton(float width, float height) {
		menu[1].setFont(font);
...
public:
	...
	Death(float width, float height) {
		...
		createNameButton();
		createTryAgainButton(width, height);
	...
	void checkResumeButton(sf::Vector2i& vec, sf::Event menu_event) {
		if (vec.x >= 925 and vec.x <= 1150 and vec.y >= 625 and vec.y <= 675) {
